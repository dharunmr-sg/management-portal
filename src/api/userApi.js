...
export async function searchUsers(query) {
  return await apiClient(`/users/search?q=${encodeURIComponent(query)}`);
}
...
export async function getUsersWithPagination(limit = 10, skip = 0) {
  return await apiClient(`/users?limit=${limit}&skip=${skip}`);
}
...
export async function getUsersByRole(role) {
  return await apiClient(`/users/filter?key=role&value=${encodeURIComponent(role)}`);
}
...
export async function getUsersByAge(minAge, maxAge) {
  const data = await apiClient(`/users?limit=200`);
